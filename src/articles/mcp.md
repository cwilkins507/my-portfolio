...
image: "/images/articles/mcp.png"
image_alt: "Model Context Protocol hub connecting AI tools to secure external capabilities."
seo_title: "Model Context Protocol (MCP): Guide for Engineers and Leaders"
meta_description: "Learn MCP (Model Context Protocol) for AI tool integration. Typed interfaces, capability security, observability, and a step-by-step example."
...
**Tools** are typed functions with explicit side-effects — think "create_ticket" or "search_docs." **Resources** work differently: they're read-only context like documents or URIs that the client can fetch without triggering any action.

Then there are **prompts** (parameterized templates you reuse across sessions) and **sessions**, which scope the whole interaction with time limits, budgets, and capability restrictions.
...
### Tool Contracts
A tool needs:
- A name and a description the model can read
- Input and output schemas (JSON Schema format)
- A side_effects boolean
- Timeout settings
- Auth scopes
...
### Observability
Log every tool invocation at the boundary. Capture:
- Tool name
- A hash of the arguments (never log raw secrets)
- Duration, status, and error codes
...
Validate everything. If the input doesn't match the schema, reject it. If the output is wrong, fail the call. Failing closed is annoying during development but saves you in production.

Cap latency and compute. Return partial results when you can. I've had agents hang for 30 seconds because a search query was too broad. Better to return 10 results fast than wait forever for 1000.

Hash resources and use ETags. Content-addressed URIs make cache invalidation simple.
...
Add capabilities over time — don't break existing ones. Use feature flags to negotiate what's available. Version bumps are coordination overhead you don't need.
...
Run golden tests offline for each tool. Check success paths and error handling. These tests run faster than integration tests and catch regressions early.
...
This minimal server shows the core MCP loop: discover, call, observe, govern.
...
Let your platform team own auth, audit, and sandboxing. Centralizing policy there means individual tool authors don't have to re-implement it.
...
Prompts that assume specific tool quirks will break when you update the implementation. Document your invariants. Make them explicit in the description field.
...
Early versions of my search tool would return 1000 results and overwhelm the client. Stream with explicit limits. The client can ask for more if it needs them.

## Measuring Success

How many days does it take to add a new tool? Before MCP, our team averaged five days. After: one day, sometimes less.

Track **error rates** by tool and reason code. You'll spot bad schemas and timeout issues fast.

Set a **p95 latency** budget for each tool. My search tool targets 400ms. When it spikes, I investigate.

Watch **cost** — tokens and compute per successful outcome. MCP doesn't eliminate costs, but structured calls waste fewer tokens than freeform API descriptions.

What percentage of calls have complete **traces and audit logs**? Aim for 100%. Gaps here mean blind spots when something goes wrong.
