...
image: "/images/articles/microservice-redesign.png"
image_alt: "Microservice redesign visual showing service boundaries, contracts, and integration paths."
...
High cognitive load. Services, contracts, failure modes—they all multiply. One team I worked on had 47 services for what could have been eight modules. Our manager noticed the same thing. We refactored to a mono-repo, but this took time.
...
- Events carry immutable facts and minimal denormalized context. Don't stuff the entire world state into an event.
- Retries can't amplify side effects.
- Every envelope carries trace data. Metrics derive from op/subject so you don't need custom instrumentation everywhere.
...
- No cross-module database access. Period.
...
1. **Inventory.** Map services, contracts, owners, SLOs, data stores, costs. You need the full picture before you start moving things.
2. **Event storming.** Document domain events and command boundaries. Get the team in a room.
3. **Define the universal interface and error taxonomy.** Build libraries and lint rules so teams can't drift.
4. **Stand up a contract registry** and schema validation in CI. Make breaking changes fail the build.
5. **Insert an adapter layer**—gateway or sidecar—that translates old APIs to the new envelope. This lets you migrate incrementally without breaking everything.
6. **Collapse low-signal services** into modules inside a shared runtime. Keep data migration incremental. We moved three services in the first month, then accelerated once the pattern was proven.
7. **Standardize authz, idempotency, tracing** at the envelope level.
8. **Retire point-to-point integrations** as you move flows to the interface.
